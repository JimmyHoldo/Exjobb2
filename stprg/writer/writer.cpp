...
int main()
{
   while(port.getPort() == -1){
       ...
       port.open_port_serial("/dev/ttyUSB3");

       if (port.getPort() == -1)
               printf("Error opening serial port /dev/ttyUSB3 \n");
           else
           {
               printf("Serial Port /dev/ttyUSB3 is Open\n");
               if (port.initport() == -1)
               {
   ...
   //std::cout << port.getPort() << std::endl;
    ...
    const int ret = dbus_bus_request_name(connection, "test.foo2.bar", DBUS_NAME_FLAG_REPLACE_EXISTING, &dbus_error);
    ...
    while (true) {
        ...
        if (dbus_message_is_method_call(msg, "test.foo2.Roll", "Method")) {
